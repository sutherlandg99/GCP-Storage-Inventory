...
# --- GCP Multi-Project Storage Audit Script (Parallelized, works on Linux, windows, and MacOS) ---
...
# Use lower number for parelellization settings below if encountering performance issues. 
...
# 4. Sufficient IAM permissions in all GCP projects (e.g., Project Viewer) on the projects to be audited.
...
MAX_CONCURRENT_BUCKETS=32  # Lower number of concurrent threads for better stability, 4 is a fairly safe number
...
process_single_bucket() {
    ...
    # Use gcloud storage du with --raw flag to get unformatted byte count
    gcloud storage du --summarize --raw "$bucket_uri" > "$temp_output" 2> "$temp_error" &
    du_pid=$!
    ...
    # First try gcloud storage du results with improved parsing for large numbers
    if [ $du_exit_code -eq 0 ] && [ -s "$temp_output" ]; then
        local bucket_size_output=$(cat "$temp_output")
        
        if [ -n "$bucket_size_output" ]; then
            # For --raw output, it should just be the number
            # But sometimes it still includes text, so we extract just digits
            bucket_bytes=$(echo "$bucket_size_output" | grep -oE '^[0-9]+' | head -1)
            
            # If that didn't work, try getting the first field
            if [[ ! "$bucket_bytes" =~ ^[0-9]+$ ]] || [ -z "$bucket_bytes" ]; then
                bucket_bytes=$(echo "$bucket_size_output" | awk '{print $1}' | grep -oE '[0-9]+' | head -1)
            fi
            
            # Final fallback: get the largest number in the output
            if [[ ! "$bucket_bytes" =~ ^[0-9]+$ ]] || [ -z "$bucket_bytes" ]; then
                bucket_bytes=$(echo "$bucket_size_output" | grep -oE '[0-9]+' | sort -n | tail -1)
            fi
    ...
    # If gcloud failed or gave 0, try gsutil results with better parsing for large numbers
    if [[ "$bucket_bytes" == "0" || "$bucket_bytes" == "" ]] && [ $gsutil_exit_code -eq 0 ] && [ -s "$temp_output_gsutil" ]; then
        ...
        if [ -n "$gsutil_output" ]; then
            # gsutil du -s output format: "12345678901234567  gs://bucket-name"
            # Extract just the first field and remove any non-digit characters
            bucket_bytes=$(echo "$gsutil_output" | awk '{print $1}' | sed 's/[^0-9]//g' | head -1)
            
            # Validate it's a proper number
            if [[ "$bucket_bytes" =~ ^[0-9]+$ ]] && [ -n "$bucket_bytes" ] && [ "$bucket_bytes" -gt 0 ]; then
    ...
    # Final validation and processing - use awk for large number division
    if [[ "$bucket_bytes" =~ ^[0-9]+$ ]] && [ "$bucket_bytes" -gt 0 ]; then
        # Use awk for division to handle very large numbers properly
        bucket_size_gb=$(awk -v bytes="$bucket_bytes" 'BEGIN {printf "%.6f", bytes/1073741824}')
        status="success"
        
        # Convert to human readable for display - use awk for large number comparisons
        local human_size size_unit
        if [ "$(awk -v b="$bucket_bytes" 'BEGIN {print (b > 1099511627776) ? 1 : 0}')" -eq 1 ]; then
            human_size=$(awk -v bytes="$bucket_bytes" 'BEGIN {printf "%.2f", bytes/1099511627776}')
            size_unit="TB"
...
process_buckets_parallel() {
    ...
    if [ -z "$bucket_list" ]; then
        echo "No GCS buckets found in project: $project_id" >&2
        echo "0"
        return 0
    fi
    ...
    echo "📦 Found $total_buckets buckets to process in parallel..." >&2
    ...
    echo "⏳ Waiting for remaining bucket scans to complete..." >&2
    for pid in "${job_pids[@]}"; do
        wait "$pid" 2>/dev/null
    done
    
    echo "✅ All bucket scans completed for project: $project_id" >&2
    
    # Collect results and update CSV - use awk for large number addition
    ...
    for ((i=1; i<=total_buckets; i++)); do
        ...
        if [ -f "$result_file" ]; then
            ...
            if [ "$status" != "processing" ]; then
                ...
                # Add to project total if it's a valid number - use awk for large numbers
                if [[ "$bucket_bytes" =~ ^[0-9]+$ ]]; then
                    project_gcs_bytes=$(awk -v total="$project_gcs_bytes" -v bytes="$bucket_bytes" 'BEGIN {printf "%.0f", total + bytes}')
                fi
            fi
...
echo "$SELECTED_PROJECTS" | while read -r project_id; do
    ...
    # Calculate project totals in TB for display - use awk for large numbers
    project_gcs_tb=$(awk -v bytes="$project_gcs_bytes" 'BEGIN {printf "%.2f", bytes/1099511627776}')
    ...
    # Add to global totals - use awk for large number addition
    TOTAL_GCS_BYTES=$(awk -v total="$TOTAL_GCS_BYTES" -v bytes="$project_gcs_bytes" 'BEGIN {printf "%.0f", total + bytes}')
...
# Calculate final totals - use awk for GCS large numbers
TOTAL_GCS_TB=$(awk -v bytes="$TOTAL_GCS_BYTES" 'BEGIN {printf "%.2f", bytes/1099511627776}')
